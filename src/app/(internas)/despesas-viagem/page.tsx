...
import {
  TipoDespesaViagem,
  adicionarVeiculo,
  adicionarDespesaViagem,
  adicionarMotorista,
  listarVeiculos,
  listarDespesasViagem,
  listarMotoristas,
  ...
  deletarDespesaViagem,
} from '@/services/despesasViagemService';
...
interface FiltroRelatorio {
  ...
  tipo: TipoDespesaViagem | 'all';
  motorista: NullableString | 'all';
  veiculo: NullableString | 'all';
}

const tipos: { value: TipoDespesaViagem; label: string }[] = [
...
export default function DespesasViagemPage() {
  ...
  const [novoMotorista, setNovoMotorista] = useState<string>('');
  const [novoVeiculo, setNovoVeiculo] = useState<string>('');
  ...
  const [prestacaoMotorista, setPrestacaoMotorista] = useState<string>('all');
  ...
  const [motoristasDocs, setMotoristasDocs] = useState<Array<{ id: string; nome: string }>>([]);
  const [veiculosDocs, setVeiculosDocs] = useState<Array<{ id: string; nome: string }>>([]);
  const [trechosDocs, setTrechosDocs] = useState<Array<{ id: string; nome: string }>>([]);
  const [editModal, setEditModal] = useState<{ type: 'motorista' | 'veiculo' | 'trecho' | null; id?: string; name: string }>({ type: null, name: '' });
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editDraft, setEditDraft] = useState<{ valor: string; tipo: string; descricao: string; motorista?: string; veiculo?: string; trecho?: string }>({ valor: '', tipo: '', descricao: '' });
  ...
  function startEditRow(l: { id: string; valor: number; tipo: string; descricao?: string; motorista?: string; veiculo?: string; trecho?: string }) {
    setEditingId(l.id);
    setEditDraft({
  ...
  async function saveEditRow() {
    ...
    try {
      ...
      await atualizarDespesaViagem(editingId, {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('viagemPrestacaoStatus');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('viagemPrestacaoStatus', prestacaoStatus);
    } catch {}
  }, [prestacaoStatus]);
  ...
  const aggMotorista = useMemo(() => aggregate((l) => l.motorista), [aggregate]);
  const aggVeiculo = useMemo(() => aggregate((l) => l.veiculo), [aggregate]);
  ...
  function applySmartExtraction(rawText: string) {
    ...
    let tipoInferido: TipoDespesaViagem | undefined;
    const hints: string[] = [];
    ...
    } else if (/(ped[aá]gio|tarifa|passagem|barreira|cobran[cç]a|via dutra|ecovia)/.test(lowered)) {
      tipoInferido = 'pedagio';
      hints.push('Detectado contexto de pedágio');
  ...
  async function handleAdicionarMotorista() {
    try {
      if (!novoMotorista.trim()) return;
      await adicionarMotorista(novoMotorista.trim());
      setNovoMotorista('');
      setAddingDriver(false);
  ...
  async function handleAdicionarVeiculo() {
    try {
      ...
      await adicionarVeiculo(novoVeiculo.trim());
      setNovoVeiculo('');
      setAddingVehicle(false);
  ...
  function openEditEntity(type: 'motorista' | 'veiculo' | 'trecho') {
    const currentName = type === 'motorista' ? form.motorista : type === 'veiculo' ? form.veiculo : form.trecho;
    if (!currentName) {
      toast.error('Selecione um item para editar');
      return;
    }
    const docs = type === 'motorista' ? motoristasDocs : type === 'veiculo' ? veiculosDocs : trechosDocs;
  ...
  async function saveEditEntity() {
    ...
    try {
      if (editModal.type === 'motorista') {
        await updateMotorista(editModal.id, newName);
        setMotoristasDocs(await listarMotoristasWithIds());
        setMotoristas(await listarMotoristas());
        setForm((s) => ({ ...s, motorista: newName }));
      } else if (editModal.type === 'veiculo') {
        await updateVeiculo(editModal.id, newName);
        setVeiculosDocs(await listarVeiculosWithIds());
        setVeiculos(await listarVeiculos());
        setForm((s) => ({ ...s, veiculo: newName }));
  ...
  async function handleSalvarDespesa() {
    try {
      ...
      const despesaId = await adicionarDespesaViagem({
        data: parsedDate,
        valor: valorNumber,
        tipo: form.tipo as TipoDespesaViagem,
  ...
  async function handleBuscarRelatorio() {
    try {
      ...
      const list = await listarDespesasViagem({
        inicio: inicioDate,
        fim: fimDate,
        tipo: filtro.tipo === 'all' ? undefined : (filtro.tipo as TipoDespesaViagem),
        motorista: filtro.motorista === 'all' ? undefined : (filtro.motorista as string | undefined),
        veiculo: filtro.veiculo === 'all' ? undefined : (filtro.veiculo as string | undefined),
      });
  ...
  async function handleBuscarPrestacao() {
    try {
      ...
      const list = await listarDespesasViagem({
        ...
        motorista: prestacaoMotorista === 'all' ? undefined : prestacaoMotorista,
  ...
  async function exportRelatorioPDF() {
    ...
    doc.setFontSize(14);
    doc.text('Relatório de Despesas - Viagem Terrestre', 14, 16);
    doc.setFontSize(10);
    const filtros = [
      filtro.inicio ? `Início: ${filtro.inicio}` : null,
      filtro.fim ? `Fim: ${filtro.fim}` : null,
      filtro.tipo !== 'all' ? `Tipo: ${filtro.tipo}` : null,
      filtro.motorista !== 'all' ? `Motorista: ${filtro.motorista}` : null,
      filtro.veiculo !== 'all' ? `Veículo: ${filtro.veiculo}` : null,
    ]
      .filter(Boolean)
      .join('  |  ');
    ...
    // @ts-ignore
    auto.default(doc, {
      head: [['Data', 'Tipo', 'Motorista', 'Veículo', 'Trecho', 'Descrição', 'Valor (R$)']],
      body: rows,
      startY: filtros ? 30 : 22,
      styles: { fontSize: 9 },
      headStyles: { fillColor: [37, 99, 235] },
    });
    doc.save('relatorio-viagem.pdf');
  }
  ...
  async function exportPrestacaoPDF() {
    ...
    const doc = new jsPDF({ orientation: 'landscape' });
    doc.setFontSize(14);
    doc.text('Prestação de Contas - Viagem Terrestre', 14, 16);
    doc.setFontSize(10);
    const filtros = [
      ...
      prestacaoMotorista !== 'all' ? `Motorista: ${prestacaoMotorista}` : null,
    ...
    // @ts-ignore
    auto.default(doc, {
      head: [['Data', 'Motorista', 'Trecho', 'Tipo', 'Descrição', 'Valor (R$)', 'Status']],
      body: rows,
      startY: filtros ? 30 : 22,
      styles: { fontSize: 9 },
      headStyles: { fillColor: [37, 99, 235] },
    });
    doc.save('prestacao-viagem.pdf');
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-gray-900">Despesas de Viagem Terrestre</h1>
        </div>
      </div>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as any)} className="w-full">
          ...
          <Card className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <Select
                  value={form.tipo}
                  onValueChange={(v) => setForm((s) => ({ ...s, tipo: v as TipoDespesaViagem }))}
                >
                  <SelectTrigger>
            ...
            {/* Seção 3: Motorista, Veículo, Trecho */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <div className="flex items-center justify-between">
                  <Label>Motorista</Label>
                  <Button variant="ghost" size="sm" onClick={() => setAddingDriver((v) => !v)}>
                    <PlusCircle className="w-4 h-4 mr-1" /> Adicionar motorista
                  </Button>
                </div>
                <Select
                  value={form.motorista || ''}
                  onValueChange={(v) => setForm((s) => ({ ...s, motorista: v }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Selecione" />
                  </SelectTrigger>
                ...
                <div className="mt-2">
                  <Button variant="outline" size="sm" onClick={() => openEditEntity('motorista')}>Editar motorista</Button>
                </div>
                {addingDriver && (
                  <div className="mt-2 flex gap-2">
                    ...
                    />
                    <Button onClick={handleAdicionarMotorista}>Salvar</Button>
                  </div>
                )}
              </div>
              ...
              <div>
                <div className="flex items-center justify-between">
                  <Label>Veículo</Label>
                  <Button variant="ghost" size="sm" onClick={() => setAddingVehicle((v) => !v)}>
                    <PlusCircle className="w-4 h-4 mr-1" /> Adicionar veículo
                  </Button>
                </div>
                <Select
                  value={form.veiculo || ''}
                  onValueChange={(v) => setForm((s) => ({ ...s, veiculo: v }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Selecione" />
                  </SelectTrigger>
                ...
                <div className="mt-2">
                  <Button variant="outline" size="sm" onClick={() => openEditEntity('veiculo')}>Editar veículo</Button>
                </div>
                {addingVehicle && (
                  <div className="mt-2 flex gap-2">
                    <Input
                      placeholder="Nome/placa do veículo"
                      value={novoVeiculo}
                      onChange={(e) => setNovoVeiculo(e.target.value)}
                    />
                    <Button onClick={handleAdicionarVeiculo}>Salvar</Button>
                  </div>
                )}
              </div>
          ...
          <Card className="p-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div>
                <Label>Motorista</Label>
                <Select
                  value={filtro.motorista || 'all'}
                  onValueChange={(v) => setFiltro((s) => ({ ...s, motorista: (v || 'all') as FiltroRelatorio['motorista'] }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Todos" />
                  </SelectTrigger>
              ...
              <div>
                <Label>Veículo</Label>
                <Select
                  value={filtro.veiculo || 'all'}
                  onValueChange={(v) => setFiltro((s) => ({ ...s, veiculo: (v || 'all') as FiltroRelatorio['veiculo'] }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Todos" />
                  </SelectTrigger>
            ...
            <div className="overflow-x-auto">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-white border rounded-lg p-4 h-64 shadow-sm">
                  <div className="text-sm text-gray-600 mb-2">Por Motorista</div>
                  <Bar data={toBarData(aggMotorista)} options={barOpts} />
                </div>
                <div className="bg-white border rounded-lg p-4 h-64 shadow-sm">
                  <div className="text-sm text-gray-600 mb-2">Por Veículo</div>
                  <Bar data={toBarData(aggVeiculo)} options={barOpts} />
                </div>
                <div className="bg-white border rounded-lg p-4 h-64 shadow-sm">
                ...
                <TableBody>
                  {linhas.map((l) => (
                    <TableRow key={l.id}>
                      ...
                      <TableCell>
                        {editingId === l.id ? (
                          <Select value={editDraft.motorista || ''} onValueChange={(v) => setEditDraft((s) => ({ ...s, motorista: v }))}>
                      ...
                      <TableCell>
                        {editingId === l.id ? (
                          <Select value={editDraft.veiculo || ''} onValueChange={(v) => setEditDraft((s) => ({ ...s, veiculo: v }))}>
        ...
        <TabsContent value="prestacao" className="mt-4">
          <Card className="p-4 space-y-4">
            <div className="text-sm text-gray-700">Controle de reembolso: motoristas pagam durante a viagem e enviam comprovantes para reembolso.</div>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
              ...
              <div>
                <Label>Motorista</Label>
                <Select value={prestacaoMotorista} onValueChange={(v) => setPrestacaoMotorista(v as any)}>
                ...
                <TableBody>
                  {linhas.map((l) => (
                    <TableRow key={`p-${l.id}`}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" onClick={async () => { await atualizarDespesaViagem(l.id, { statusReembolso: 'aprovado' }); toast.success('Status atualizado'); handleBuscarPrestacao(); }}>Aprovar</Button>
                          <Button size="sm" variant="outline" onClick={async () => { await atualizarDespesaViagem(l.id, { statusReembolso: 'pago' }); toast.success('Status atualizado'); handleBuscarPrestacao(); }}>Marcar pago</Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
                ...
                onClick={async () => {
                  try {
                    if (confirmModal.action === 'aprovar') {
                      await Promise.all(
                        linhas.map((l) => atualizarDespesaViagem(l.id, { statusReembolso: 'aprovado' }))
                      );
                      toast.success('Todos os registros foram aprovados');
                    } else if (confirmModal.action === 'pagar') {
                      await Promise.all(
                        linhas.map((l) => atualizarDespesaViagem(l.id, { statusReembolso: 'pago' }))
                      );
