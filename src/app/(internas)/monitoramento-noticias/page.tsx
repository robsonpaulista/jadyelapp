...
async function getNews(forceRefresh: boolean = false, feedType: 'jadyel' | 'deputados' = 'jadyel'): Promise<NewsItem[]> {
  try {
    ...
    const feedParam = `&feed=${feedType}`;
    const res = await fetch(`/api/noticias?t=${timestamp}${refreshParam}${feedParam}`, {
      cache: 'no-store',
      headers: {
...
export default function MonitoramentoNoticiasPage() {
  ...
  const loadNews = async (forceRefresh: boolean = false, feedType: 'jadyel' | 'deputados' = 'jadyel') => {
    ...
    try {
      const data = await getNews(forceRefresh, feedType);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="py-4">
              <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                ...
                <TabsContent value="jadyel" className="mt-4">
